...
```bash
.venv311/bin/python scripts/prepare_real_baseline.py --records 384 --duration-seconds 2.0
.venv311/bin/python scripts/validate_manifest.py data/manifests/voca_real_v1/train.jsonl
.venv311/bin/python scripts/train_manifest.py data/manifests/voca_real_v1/train.jsonl \
  --epochs 1 --batch-size 4 --backend fallback \
  --output checkpoints/voca-real-v1-fallback.pt
.venv311/bin/python scripts/evaluate_manifest.py \
