...
class ContrastiveTSELoss(torch.nn.Module):
    def __init__(self, speaker_encoder, lambda_contrastive=0.1, lambda_leakage=0.1,
                 lambda_level=0.05, margin=0.4):
        super().__init__()
    ...
    def forward(self, target, estimate, negative=None, return_components=False):
        si_sdr = si_sdr_loss(target, estimate)
        target_rms = torch.sqrt(target.square().mean(dim=-1) + 1e-8)
        estimate_rms = torch.sqrt(estimate.square().mean(dim=-1) + 1e-8)
        level = (torch.log(estimate_rms) - torch.log(target_rms)).abs().mean()
        total = si_sdr + self.lambda_level * level
        ...
        if return_components:
            return total, {"si_sdr": si_sdr.detach(), "level": level.detach(),
                           "contrastive": contrastive.detach(), "leakage": leakage.detach()}
        return total
